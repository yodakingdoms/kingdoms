...
void create_object(void)
{
    ...
    set_long("You are on a road through the forest. The huge oak trees " +
             ...
             "it, almost creating a roof over your head. Only very " +
             "little sunlight filters down through the branches, so it " +
             "is rather shadowy here. The road continues northeast and " +
             "south. There is a sign beside the road.\n");
    set_new_light(3);
    ...
    add_item("sign|@sign","'Shrine and Stone hill stone quarry to the " +
             "north'");
    add_item("$|$sound|$soft sound|$wind","The only thing you hear is " +
             "the soft sound of the wind in the trees");

    add_exit(ROOM + "north_road_path","south");
    add_exit(ROOM + "north_road8","northeast");    
}
