...
void create_object(void)
{
    ...
    set_long("You are on a road through the forest. The forest is not " +
             ...
             "moving in the silent forest around you. The main road " +
             "continues to the northwest, and a side road goes " +
             "northeast.\n");
    set_new_light(5);
    ...
    add_item("shadows|shadow","Who knows what may be lurking in the " +
             "shadows? Maybe you shouldn't stick around to find out..");
    add_item("$|$sound|$soft sound|$wind","The only thing you hear is " +
             "the soft sound of the wind in the trees");           
             
    add_exit(ROOM + "north_road8","southwest");
    add_exit(ROOM + "north_road11","northeast");
    add_exit(ROOM + "north_road10","northwest");                 
}
